...
=========================
   ...
   corner of jamovi), select ‘Open’ and then ‘Examples’ or ‘This PC’,
...
.. A very common format for exchanging data is using files with ‘comma-
   ...
   nearly any software package can read and write them (and any simple
   text editor permits you to take a look). An example is shown in the
   figure. The first line contains the variable names and any other line
   afterwards a measurement point (typically a person where data were
   acquired from). If you have your data in a spreadsheet program (e.g.,
   Excel), you can create a CSV-file from your data by selecting ``File``
   → ``Save as...`` and choose ``Comma delimited`` in the drop-down menu
   close to the ``Save as``-button. Thereafter, you may open the CSV-file
   in jamovi.
  
   If jamovi does not open a file correctly, do file an issue (bug
   report) in our GitHub `issue tracker <https://github.com/jamovi/jamovi/issues>`__.
...
.. An overview with short, non-technical tutorials on how to do common
   procedures in jamovi can be found under jamovi-guide (later in this
   section).
...
===========
   ...
   mode, select the Application menu (⋮) at the top right of jamovi,
   and check the “Syntax mode” checkbox there. It is possible to leave
   syntax mode by clicking this a second time.
   ...
   <https://blog.jamovi.org/2018/07/30/rj.html>`__, which allows you
   to use R code to analyse data directly in jamovi.
